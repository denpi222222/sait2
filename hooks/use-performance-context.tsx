...
export function PerformanceProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Enable performance mode by default on mobile or slow devices
    const shouldEnablePerformanceMode = mobile || slowDevice;
    ...
    if (savedPreference !== null) {
      ...
      // Default to performance mode on mobile/slow devices
      setIsLiteMode(shouldEnablePerformanceMode);
      localStorage.setItem('performanceMode', shouldEnablePerformanceMode.toString());
    }
    ...
    const memory = typeof navigator !== 'undefined' && (navigator as any).deviceMemory ? (navigator as any).deviceMemory : 0;
    ...
    if (connection) {
      if (connection.effectiveType === 'slow-2g' || connection.effectiveType === '2g') connectionSpeed = 'slow';
      else if (connection.effectiveType === '3g') connectionSpeed = 'average';
      else if (connection.effectiveType === '4g') connectionSpeed = 'fast';
    }

    setPerformanceInfo({ hardwareConcurrency: cores, deviceMemory: memory, connectionSpeed });
  }, []);
