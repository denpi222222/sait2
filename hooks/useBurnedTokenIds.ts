...
export function useBurnedTokenIds(){
  ...
  useEffect(()=>{
    const run = async()=>{
      if(!isConnected || !address){setTokenIds([]);return}
      ...
      try{
        const sizeBig = await publicClient.readContract({address:GAME_ADDRESS,abi:GAME_ABI,functionName:'totalBurned'}) as bigint
        const size = Number(sizeBig)
        const set = new Set<string>()
        for(let i=0;i<size;i++){
          const tid:bigint = await publicClient.readContract({address:GAME_ADDRESS,abi:GAME_ABI,functionName:'graveyardTokens',args:[BigInt(i)]}) as bigint
          set.add(tid.toString())
        }
        // logs
        try{
          const logs = await publicClient.getLogs({address:GAME_ADDRESS,event:NFT_BURNED_EVENT,args:{player:address}}) as any[]
