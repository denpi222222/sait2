...
export const useNFTGameInfo = (tokenId: string | undefined) => {
  ...
  const { pingInterval, breedCooldown: globalBreedCooldown } = useCrazyCubeGame()
  ...
  const formatNFTInfo = (): NFTGameInfo | null => {
    ...
    const pingCooldown = Math.max(0, (Number(nftState[2]) + pingSec) - currentTime)
    const breedCooldown = Math.max(0, (Number(nftState[3]) + breedSec) - currentTime)
    ...
    return {
      ...
      lockedCRA: nftState[1].toString(),
      lockedCRAFormatted: formatEther(nftState[1]),
...
export const useBurnRecord = (tokenId: string | undefined) => {
  ...
  const { data: burnRecord, isLoading, error, refetch } = useReadContract({
    address: GAME_CONTRACT_ADDRESS,
    abi: GAME_CONTRACT_ABI,
  ...
  const formatBurnRecord = (): BurnRecordInfo | null => {
    ...
    const canClaim = !burnRecord[4] && timeLeft === 0 && Number(burnRecord[1]) > 0
...
export const useMultipleNFTGameInfo = (tokenIds: string[]) => {
  ...
  const { data: nftDataResults, isLoading: isLoadingData, error: dataError, refetch: refetchData } = useReadContracts({
    contracts: dataContracts,
    query: { enabled }
  })

  const { data: nftStateResults, isLoading: isLoadingState, error: stateError, refetch: refetchState } = useReadContracts({
  ...
  const formatMultipleNFTInfo = (): NFTGameInfo[] => {
    ...
    return tokenIds.map((tokenId, index) => {
      ...
      return {
        ...
        canBreed: nftData[2] && !nftState[4] && Number(nftState[0]) > 0 && breedCooldown === 0,
        pingCooldown,
        breedCooldown
      }
