 ...
 */
export function useSocialPrompt(wallet: string | undefined, pingedJustNow: boolean) {
  const [show, setShow] = useState(false)

  // Helper to close + mark last prompt timestamp
  const close = useCallback(() => {
    if (wallet) {
      localStorage.setItem(`lastSocialPrompt:${wallet}`, new Date().toISOString())
    }
    setShow(false)
  }, [wallet])
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("social prompt localStorage", err)
    }
