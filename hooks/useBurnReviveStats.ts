...
const BURN_TOPIC = "0x395e82a632c09e2530de3268c938c809f5d457f41726b2dfcae6ea8657fca2cc" // keccak256("Burn(address,uint256,uint8)")
const REVIVE_TOPIC = "0x3df601c7a509a68e2db279b1091d651d81d77ec2a3f8b80b9dcd51cb07cdbcfa" // keccak256("Revive(address,uint256)")

export interface SeriesPoint { date: string; burn: number; revive: number }
      ...
      try{
        ...
        const buildSeries=(days:number)=>{
          ...
          for(let i=days-1;i>=0;i--){
            const d=new Date((now-86400*i)*1000).toISOString().slice(0,10)
            arr.push({date:d,burn:burnMap.get(d)||0,revive:reviveMap.get(d)||0})
          }
          return arr
        }
