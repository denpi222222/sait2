...
const GAME_STATS_ABI = [
  ...
  { name: 'totalBurnedCRA', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'uint256' }] },
  { name: 'rewardRatePerSecond', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'uint256' }] },
  { name: 'pingInterval', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'uint256' }] },
  { name: 'breedCooldown', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'uint256' }] },
  { name: 'graveyardCooldown', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'uint256' }] },
  { name: 'burnFeeBps', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'uint256' }] },
  { name: 'manualFloorPrice', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'uint256' }] },
  { name: 'monthlyUnlockPercentage', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'uint256' }] },
  { name: 'perPingCapDivisor', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'uint256' }] },
  { name: 'paused', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'bool' }] },
  { name: 'nftContract', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'address' }] },
  { name: 'craToken', type: 'function', stateMutability: 'view', inputs: [], outputs: [{ type: 'address' }] }
] as const
...
export const useGameStats = () => {
  ...
  const fetchGameStats = async () => {
    ...
    try {
      setError(null)
      console.log('🔄 Fetching consolidated game stats...')

      // First get contract addresses
      const [nftAddress, craAddress] = await Promise.all([
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'nftContract' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'craToken' })
      ...
      const [
        ...
        // Game contract stats
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'totalBurnedCRA' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'totalBurned' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'monthlyRewardPool' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'totalLockedForRewards' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'mainTreasury' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'getBreedCostCRA' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'rewardRatePerSecond' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'pingInterval' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'breedCooldown' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'graveyardCooldown' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'burnFeeBps' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'manualFloorPrice' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'monthlyUnlockPercentage' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'perPingCapDivisor' }),
        publicClient.readContract({ address: GAME_ADDR, abi: GAME_STATS_ABI, functionName: 'paused' }),
        // NFT contract stats
        publicClient.readContract({ address: nftAddress, abi: ERC721_ABI, functionName: 'totalSupply' }),
        // CRA token stats
        publicClient.readContract({ address: craAddress, abi: ERC20_ABI, functionName: 'totalSupply' }),
        publicClient.readContract({ address: craAddress, abi: ERC20_ABI, functionName: 'balanceOf', args: [DEAD_ADDRESS] })
      ])
      ...
      const monthlyUnlockPercent = Number(monthlyUnlockPercentage as bigint) / 100
      ...
      const gameStats: GameStats = {
        ...
        craTotalSupply: formatEther(craTotalSupply as bigint),
        craDeadBalance: formatEther(craDeadBalance as bigint),
        burnFeePercentage: burnFeePercent,
        monthlyUnlockPercent: monthlyUnlockPercent,
        lastUpdated: Date.now()
      }

      console.log('✅ Consolidated game stats loaded:', {
        totalNFTs: gameStats.totalNFTs,
        activeCubes: gameStats.activeCubes,
        graveyardSize: gameStats.graveyardSize,
        contractData: `totalSupply=${totalNFTCount}, totalBurned=${burnedCount}`,
        currentMonthlyPool: gameStats.currentMonthlyPool,
        isPaused: gameStats.isPaused
      })
