    ...
    <div className="fixed inset-0 pointer-events-none z-20">
      {particles.map(particle => (
        <motion.div
          ...
          animate={{
            x: centerX + Math.cos(particle.angle + Date.now() * 0.0001 * particle.speed) * particle.radius,
            y: centerY + Math.sin(particle.angle + Date.now() * 0.0001 * particle.speed) * particle.radius,
          }}
          transition={{
            duration: 0,
            ease: "linear"
...
const TextCube = ({ char, index, position }: { 
  ...
  if (isDestroyed) {
    return (
      <div className="absolute" style={{ transform: `translate(${position.x}px, ${position.y}px)` }}>
      ...
      <div className="relative w-20 h-20 md:w-24 md:h-24" style={{ transformStyle: "preserve-3d" }}>
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-purple-500 via-pink-500 to-cyan-500 border-2 border-white/30 flex items-center justify-center text-white font-black text-4xl md:text-5xl shadow-lg overflow-hidden"
          style={{ 
            ...
            textShadow: '0 0 10px rgba(255,255,255,0.8), 0 0 20px rgba(0,255,255,0.6), 0 0 30px rgba(255,0,255,0.4)'
          }}>
