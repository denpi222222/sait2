...
export default function BreedPage() {
  ...
  // Function to set cooldown on bred NFTs (55 seconds for parents)
  const addBredNFTsCooldown = useCallback((usedTokenIds: number[]) => {
    const now = Date.now();
    const cooldownDuration = 55 * 1000; // 55 секунд в миллисекундах
  ...
  const refreshDataAfterBreeding = useCallback(
    (usedTokenIds: number[]) => {
      // Set cooldown on used NFTs (55 seconds for parents)
  ...
  const handleSelectNFT = (tokenId: number) => {
    ...
    // Check if NFT is on cooldown (55 seconds for parents)
    const now = Date.now();
            ...
            <div className='space-y-8'>
              ...
              >
                ...
                {userNFTs.map((nft, index) => {
                  ...
                  // Check if NFT is on breeding cooldown (55 seconds for parents)
                  const now = Date.now();
