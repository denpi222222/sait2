...
export default function PlayerAnalytics() {
  ...
  const fetchPlayerStats = async () => {
    try {
      ...
      const activePlayers = players.filter((p: Player) => 
        parseInt(p.lastActivityAt) >= sevenDaysAgo
      ).length;
  ...
  const getActivityLevel = (player: Player) => {
    ...
    if (total >= 50) return { level: 'Legendary', color: 'text-yellow-400', bg: 'bg-yellow-900/20' };
    if (total >= 20) return { level: 'Expert', color: 'text-purple-400', bg: 'bg-purple-900/20' };
    if (total >= 10) return { level: 'Active', color: 'text-blue-400', bg: 'bg-blue-900/20' };
    if (total >= 5) return { level: 'Casual', color: 'text-green-400', bg: 'bg-green-900/20' };
    ...
    return (
      <Card className="p-6 bg-slate-800/50 backdrop-blur-sm border-slate-700">
        <div className="flex items-center justify-center h-32">
          ...
          <span className="ml-3 text-slate-300">Loading player analytics...</span>
        </div>
      </Card>
    );
  }
      ...
      <Card className="p-6 bg-gradient-to-r from-blue-900/50 to-purple-900/50 backdrop-blur-sm border-blue-500/30">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-green-400">{stats.activePlayers}</p>
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <Badge className={getActivityLevel(stats.playerSearch).bg + ' ' + getActivityLevel(stats.playerSearch).color}>
                  {getActivityLevel(stats.playerSearch).level}
                </Badge>
              </div>
              <div className="text-right">
                <p className="text-slate-400 text-sm">First activity</p>
                <p className="text-slate-300">{formatTime(stats.playerSearch.firstSeenAt)}</p>
              </div>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <p className="text-blue-400 text-2xl font-bold">{stats.playerSearch.totalPings}</p>
                <p className="text-slate-400 text-sm">Pings</p>
              </div>
              <div className="text-center">
                <p className="text-red-400 text-2xl font-bold">{stats.playerSearch.totalBurns}</p>
                <p className="text-slate-400 text-sm">Burns</p>
              </div>
              <div className="text-center">
                <p className="text-green-400 text-2xl font-bold">{stats.playerSearch.totalBreeds}</p>
                <p className="text-slate-400 text-sm">Breeds</p>
              </div>
              <div className="text-center">
                <p className="text-yellow-400 text-2xl font-bold">{stats.playerSearch.totalClaims}</p>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <Badge className={getActivityLevel(player).bg + ' ' + getActivityLevel(player).color + ' text-xs'}>
                ...
                <div className="flex items-center space-x-4 text-sm">
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="text-yellow-400">{player.totalClaims}</span>
                  </div>
                </div>
              </div>
            </motion.div>
