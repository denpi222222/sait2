...
export default function SubgraphStats({ address }: { address?: string }) {
  ...
  useEffect(() => {
    async function fetchGlobalStats() {
      try {
        ...
        if (errors || !data || !data.globalStats) {
          console.warn("Subgraph error or no data:", errors || "No data returned");
    ...
    Promise.all([fetchGlobalStats(), fetchRecentBreeds()])
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 border rounded-lg">
                ...
                <div className="text-2xl font-bold">{globalStats.totalBurns}</div>
              </div>
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-gray-500">Total bred</div>
                <div className="text-2xl font-bold">{globalStats.totalBreeds}</div>
              </div>
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-gray-500">Total pings</div>
                <div className="text-2xl font-bold">{globalStats.totalPings}</div>
              </div>
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-gray-500">Active NFTs</div>
                <div className="text-2xl font-bold">{globalStats.totalActiveNFTs}</div>
              </div>
              <div className="p-4 border rounded-lg">
                <div className="text-sm text-gray-500">In graveyard</div>
                <div className="text-2xl font-bold">{globalStats.totalInGraveyard}</div>
              </div>
            </div>
          ) : (
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              ...
              {recentBreeds.map(breed => (
                <div key={breed.id} className="grid grid-cols-5 gap-2 text-sm p-2 border-b">
          ...
          <TabsContent value="user">
            {loading ? (
              ...
              <div className="space-y-2">
                ...
                {userNFTs.map(nft => (
                  <div key={nft.id} className="grid grid-cols-5 gap-2 text-sm p-2 border-b">
                    <div>#{nft.tokenId}</div>
                    <div>{nft.rarity || 'N/A'}</div>
                    <div>{nft.currentStars || 0}</div>
                    <div>{nft.isInGraveyard ? '💀 Graveyard' : '✅ Active'}</div>
                    <div>{nft.lockedCRA ? new Intl.NumberFormat('en-US', { 
                      minimumFractionDigits: 2, 
                      maximumFractionDigits: 2 
                    }).format(parseInt(nft.lockedCRA) / 1e18) : '0'}</div>
                  </div>
                ))}
              </div>
            ) : (
