...
export function DistributionChart() {
  ...
  return (
    <ChartCard title="NFT Distribution" icon={<PieChart className="h-5 w-5" />} color="purple" loading={isLoading}>
      {/* Tabs */}
        ...
        <div className="relative w-44 h-44">
          <svg viewBox="0 0 100 100" className="w-full h-full rotate-[-90deg]">
            {chartData.map((item, index) => {
              ...
              const offset = chartData.slice(0, index).reduce((sum, d) => sum + d.value / total, 0);
      ...
      <div className="flex flex-col mt-4 space-y-2">
        {chartData.map((item) => (
          <div key={item.name} className="flex items-center justify-between">
            ...
            <div className="flex items-center">
              <span className="text-sm font-medium text-slate-200 mr-1">{item.value}</span>
              ...
              <div key={item.rarity} className="text-center">
                ...
                <div className="flex h-2 mt-1 overflow-hidden rounded-full bg-slate-700/30">
                  ...
                  <div 
                    ...
                    style={{ width: `${item.inGraveyard / item.count * 100}%` }}
                  ></div>
                </div>
