...
interface UserNFTInfo {
  ...
  rarity: number;
  stars: number;
  lockedCRA: string; // formatted ether string
  lockedCRAWei: bigint; // raw wei for math
  isInGraveyard?: boolean;
  image?: string;
}

const getRarityNames = (t: any) => [
...
export default function UserNftsList() {
  ...
  const [totalLockedCRA, setTotalLockedCRA] = useState<string>("0");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        alchemyNfts.forEach((nft, idx) => {
          ...
          if (data) {
            tokens.push({
              ...
              lockedCRA: data.lockedCRA,
              lockedCRAWei: data.lockedCRAWei,
              isInGraveyard: data.isInGraveyard,
              image: typeof (nft as any).image === 'string'
                ? (nft as any).image
                : (nft as any).image?.cachedUrl || (nft as any).image?.pngUrl || (nft as any).image?.originalUrl || ((nft as any).media?.[0]?.gateway ?? ""),
            });
            totalLocked += data.lockedCRAWei;
          } else {
            tokens.push({
              tokenId,
              rarity: (nft as any).rarity || 0,
              stars: (nft as any).stars || 0,
              lockedCRA: "0",
              lockedCRAWei: 0n,
              isInGraveyard: (nft as any).isInGraveyard || nft.frozen,
              image: typeof (nft as any).image === 'string'
                ? (nft as any).image
                : (nft as any).image?.cachedUrl || (nft as any).image?.pngUrl || (nft as any).image?.originalUrl || ((nft as any).media?.[0]?.gateway ?? ""),
            });
        ...
        setNfts(tokens);
        setTotalLockedCRA(formatEther(totalLocked));
      } catch (e: any) {
        console.error("UserNftsList error", e);
        setError("Failed to load NFTs. Please reload the page or try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!isConnected) {
    return (
      <Card className="p-4 bg-slate-800/50 border-slate-700">
        <p className="text-slate-300 text-center text-sm">{t('info.connectWalletNfts', 'Connect your wallet to see your NFTs.')}</p>
      </Card>
    );
  }
    ...
    <div className="space-y-4">
      <Card className="p-4 bg-slate-800/50 border-slate-700">
        {loading || alchemyLoading ? (
          ...
          <p className="text-slate-300 text-sm text-center">{t('info.noNfts', 'You don\'t own any CrazyCube NFTs.')}</p>
        ) : (
          <>
            <h3 className="text-lg font-bold text-white mb-3 text-center">
              ...
              {liveNfts.length > 0 && graveyardNfts.length > 0 && (
                <span className="text-sm text-slate-400 ml-2">
                  ({liveNfts.length} {t('info.live', 'live')}, {graveyardNfts.length} {t('info.inGraveyard', 'in graveyard')})
                </span>
              )}
            </h3>
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                        <span className="text-slate-400 text-xs flex items-center gap-1">
                          {nft.image && (
                            <img src={nft.image} alt="thumb" className="w-8 h-8 rounded-sm object-cover" />
                          )}
                          NFT #{nft.tokenId}
                        </span>
                  ...
                  <div className="flex items-center text-yellow-400 text-sm mb-1">
                    {Array.from({ length: nft.stars }).map((_, i) => (
                      <Star key={i} className="h-4 w-4 fill-yellow-400" />
                        ...
                        {nft.stars === 0 && <span className="text-gray-500 text-xs">{t('info.noStars', 'No stars')}</span>}
                  </div>
                  <div className="flex items-center text-green-400 text-sm">
                    <Lock className="h-4 w-4 mr-1" />
                        {t('info.lockedCra', 'Locked CRA')}: {Number(nft.lockedCRA).toFixed(4)}
                      </div>
                    </motion.div>
                  ))}
                </div>
                  ...
                  {graveyardNfts.map((nft, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center text-gray-500 text-sm mb-1">
                        <span className="text-xs">💀 {t('info.inGraveyard', 'In Graveyard')}</span>
                      </div>
      ...
      {nfts.length > 0 && Number(totalLockedCRA) > 0 && (
        <Card className="p-6 bg-gradient-to-r from-yellow-900/30 to-orange-900/30 border-2 border-yellow-500/50 text-center shadow-lg shadow-yellow-500/20">
          <div className="mb-4">
            <h3 className="text-2xl font-bold text-yellow-300 mb-2">
              💰 {t('info.totalCraLocked', 'Total CRA locked in your NFTs')}
            </h3>
            <div className="text-4xl font-black text-white bg-gradient-to-r from-yellow-400 to-orange-400 bg-clip-text text-transparent">
              {Number(totalLockedCRA).toFixed(4)} CRA
            </div>
          </div>
          <div className="bg-yellow-500/10 border border-yellow-400/30 rounded-lg p-4 mt-4">
            <p className="text-yellow-200 text-base font-semibold leading-relaxed">
              ⚠️ {t('info.craLockedWarning', 'CRA locked inside an NFT are not burned and travel with the NFT when it is transferred, sold or moved in any way. They permanently belong to the NFT itself. You can obtain these tokens only by')} <span className="text-red-400 font-bold underline">{t('info.craLockedWarningBurn', 'burning')}</span> NFT.
            </p>
          </div>
        </Card>
      )}
