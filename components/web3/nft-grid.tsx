...
interface NFTGridProps {
  ...
  selectable?: boolean
  maxDisplay?: number
  CardComponent?: (props:{nft:NFT,delay:number,onSelect?: (nft:NFT)=>void,selectable?:boolean})=>JSX.Element
}

export function NFTGrid({ onSelect, selectable = false, maxDisplay, CardComponent }: NFTGridProps) {
  ...
  if (nfts.length === 0) {
    return (
      <div className="bg-slate-900/20 border border-slate-500/50 rounded-lg p-8 text-center">
        ...
        <p className="text-sm text-slate-400/70 mt-2">Connect your wallet or mint some NFTs to get started</p>
      </div>
    )
  }
