...
const DENIS_3_ENDPOINT = 'https://api.studio.thegraph.com/query/111010/denis-3/v0.0.3';
                ...
                <Card className="p-4 bg-blue-900/20 border-blue-500/30">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-white">{formatNumber(globalStats.totalPings)}</p>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-orange-400 font-semibold">{formatCRA(contractStats.totalCRABurned)} CRA</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">CRA distributed:</span>
                    <span className="text-green-400 font-semibold">{formatCRA(contractStats.totalCRADistributed)} CRA</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Floor Price:</span>
                    <span className="text-blue-400 font-semibold">{formatCRA(contractStats.manualFloorPrice)} CRA</span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-purple-400 font-semibold">{formatNumber(contractStats.totalActivations)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Total claims:</span>
                    <span className="text-green-400 font-semibold">{formatNumber(contractStats.totalClaims)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Graveyard size:</span>
                    <span className="text-red-400 font-semibold">{formatNumber(contractStats.graveyardSize)}</span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-slate-300 text-sm">{formatTime(contractStats.lastUpdated)}</span>
                  </div>
                </div>
              </div>
            </Card>
          ...
          {contractStats && (
            <Card className="p-6 bg-slate-800/50 backdrop-blur-sm border-slate-700">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-blue-900/20 rounded-lg p-4 border border-blue-500/30">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-blue-300 font-semibold">Ping Cooldown</span>
                        <Zap className="h-5 w-5 text-blue-400" />
                      </div>
                      <p className="text-2xl font-bold text-white">{cooldowns.ping}</p>
                      <p className="text-blue-300 text-sm mt-1">Between NFT pings</p>
                    </div>

                    <div className="bg-green-900/20 rounded-lg p-4 border border-green-500/30">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-green-300 font-semibold">Breed Cooldown</span>
                        <Heart className="h-5 w-5 text-green-400" />
                      </div>
                      <p className="text-2xl font-bold text-white">{cooldowns.breed}</p>
                      <p className="text-green-300 text-sm mt-1">Between breeds</p>
                    </div>

                    <div className="bg-red-900/20 rounded-lg p-4 border border-red-500/30">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-red-300 font-semibold">Graveyard Cooldown</span>
                        <Skull className="h-5 w-5 text-red-400" />
                      </div>
                      <p className="text-2xl font-bold text-white">{cooldowns.graveyard}</p>
                      <p className="text-red-300 text-sm mt-1">In Graveyard</p>
                    </div>

                    <div className="bg-purple-900/20 rounded-lg p-4 border border-purple-500/30">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-purple-300 font-semibold">Max Accumulation</span>
                        <Timer className="h-5 w-5 text-purple-400" />
                      </div>
                      <p className="text-2xl font-bold text-white">{cooldowns.maxAccumulation}</p>
                      <p className="text-purple-300 text-sm mt-1">Max accumulation</p>
                    </div>
                  </div>
                );
              })()}
              ...
              <div className="mt-6 p-4 bg-slate-900/50 rounded-lg">
                <h4 className="text-lg font-semibold text-slate-200 mb-3">Cooldown explanation:</h4>
                <div className="space-y-2 text-sm text-slate-300">
                  <p><strong className="text-blue-400">Ping Cooldown:</strong> Time between pings of a single NFT to earn rewards</p>
                  <p><strong className="text-green-400">Breed Cooldown:</strong> Time between breeds (NFT needs rest after breeding)</p>
                  <p><strong className="text-red-400">Graveyard Cooldown:</strong> Waiting time in graveyard before claiming</p>
                  <p><strong className="text-purple-400">Max Accumulation:</strong> Maximum reward accumulation time without ping</p>
                </div>
              </div>
            </Card>
          )}
                  ...
                  {recentBurns.map((burn, index) => (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-white font-semibold">NFT #{burn.tokenId}</p>
                          <p className="text-slate-400 text-sm">by {formatAddress(burn.player)}</p>
                          <p className="text-red-400 text-sm">{formatCRA(burn.amountToClaim)} CRA</p>
                        </div>
                ...
                <div className="space-y-3">
                  {recentPings.map((ping, index) => (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-white font-semibold">NFT #{ping.tokenId}</p>
                          <p className="text-slate-400 text-sm">by {formatAddress(ping.player)}</p>
                          <p className="text-blue-400 text-sm">+{formatCRA(ping.reward)} CRA</p>
                        </div>
                        <div className="text-right">
        ...
        <TabsContent value="players" className="space-y-6">
          <Card className="p-6 bg-slate-800/50 backdrop-blur-sm border-slate-700">
            ...
            {topPlayers.length > 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-700">
                      ...
                      <th className="text-right py-3 text-slate-400">Last activity</th>
                    </tr>
                  </thead>
                  <tbody>
              ...
              <p className="text-slate-400 text-center py-8">Player data is loading...</p>
            )}
          </Card>
        </TabsContent>
      </Tabs>
