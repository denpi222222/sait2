...
import { Code2, Hash, Layers, Sparkles, Copy, ExternalLink } from "lucide-react"
...
export function ContractInfo({ nftCount, craTotal }: Props) {
  ...
  const CRA_ADDRESS = apeChain.contracts.crazyToken.address
  ...
  const ContractCard = ({ 
    ...
    <motion.div
      ...
      className="group"
    >
      <Card className={`relative overflow-hidden border-2 bg-gradient-to-r ${gradientColors} p-4 transition-all duration-300 hover:scale-105 hover:shadow-xl`}>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <ContractCard
          label={t("info.tabs.craToken", "CRA Token")}
          address={CRA_ADDRESS}
          icon={Sparkles}
          ...
          {nftCount !== undefined && (
            <Card className="bg-gradient-to-r from-emerald-500/20 to-green-600/20 border-emerald-300/30 p-4">
              <div className="flex items-center">
                ...
                <div>
                  <div className="text-slate-300 text-sm">{t("info.totalNfts", "Total NFTs")}</div>
                  <div className="text-white font-bold text-xl">{nftCount.toLocaleString()}</div>
                </div>
              </div>
            </Card>
          )}
          ...
          {craTotal && (
            <Card className="bg-gradient-to-r from-yellow-500/20 to-amber-600/20 border-yellow-300/30 p-4">
              <div className="flex items-center">
                ...
                <div>
                  <div className="text-slate-300 text-sm">{t("info.craSupply", "CRA Supply")}</div>
                  <div className="text-white font-bold text-xl">{craTotal} CRA</div>
                </div>
              </div>
            </Card>
          )}
