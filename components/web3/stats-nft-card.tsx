...
const resolveImage=(url?:string|null)=>{
  ...
  if(url.startsWith("ipfs://")) return `https://nftstorage.link/ipfs/${url.slice(7)}`
  return url
}
...
export default function StatsNFTCard({nft,delay=0,onSelect}:Props){
  ...
  if(nft.stars){
    widgets.push(<span key="stars" className="flex space-x-0.5">{Array.from({length:5}).map((_,idx)=>(<Star key={idx} className={`w-3 h-3 ${idx < (nft.stars||0)?'text-yellow-400 fill-current':'text-slate-600'}`} />))}</span>)
  }
  return (
    <UnifiedNftCard
      ...
      tokenId={nft.tokenId}
      title={nft.name}
      rarityLabel={getLabel(nft.stars||1)}
