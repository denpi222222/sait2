...
const MarketPrices = () => {
  ...
  const fetchMarketData = async () => {
    try {
      ...
      if (marketResult.error) {
        console.warn('Market API returned error:', marketResult.error);
        // Use fallback data if available
        if (marketResult.floorApe || marketResult.craUsd || marketResult.apeUsd) {
      ...
      try {
        const craResponse = await fetch('/api/cra-token');
        if (craResponse.ok) {
          const craResult = await craResponse.json();
          if (craResult.success && craResult.data) {
            setCraData(craResult.data);
          }
        }
      } catch (craError) {
        console.warn('CRA token data unavailable:', craError);
  ...
  const formatPrice = (price: number | null | undefined, decimals: number = 8) => {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              <div className={`h-2 w-2 rounded-full ${error ? 'bg-red-400' : 'bg-green-400 animate-pulse'}`}></div>
              <div className="text-right">
                <p className={`text-sm ${error ? 'text-red-400' : 'text-green-400'}`}>
                  {error ? t('market.error') : t('market.live')}
                </p>
                <p className="text-slate-400 text-xs">
                  {lastUpdate.toLocaleTimeString(i18n.language === 'ru' ? 'ru-RU' : 'en-US')}
                </p>
              </div>
            </div>
          </div>
          ...
          <Card className="p-6 bg-gradient-to-br from-purple-900/20 to-pink-900/20 border-purple-500/30">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-purple-300 text-sm">{t('market.floorPrice')}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-purple-500/20">
              ...
              >
                ...
                <span className="text-white font-bold text-base">{t('market.buyNft')}</span>
          ...
          <Card className="p-6 bg-gradient-to-br from-orange-900/20 to-red-900/20 border-orange-500/30">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-orange-300 text-sm">{t('market.gameToken')}</p>
                </div>
              </div>
              {craData && (
                <Badge 
                ...
                >
                  <div className="flex items-center">
                    ...
                    <span className="ml-1">
                      {craData.price_change_24h > 0 ? '+' : ''}{craData.price_change_24h.toFixed(2)}%
                    </span>
                  </div>
                </Badge>
              )}
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-bold text-orange-400">
                  ${formatPrice(craData ? craData.price_usd : marketData.craUsd, 10)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400">APE:</span>
                <span className="text-xl font-semibold text-white">
                  {craData ? formatPrice(craData.price_ape, 8) : formatPrice(marketData.craUsd / marketData.apeUsd, 8)} APE
                </span>
              </div>
              {craData && (
                <div className="flex justify-between items-center">
            ...
            <div className="mt-4 pt-4 border-t border-orange-500/20">
              ...
              >
                ...
                <span className="text-white font-bold text-base">{t('market.tradeDex')}</span>
                <ExternalLink className="h-4 w-4 ml-2 text-white" />
              </a>
            </div>
          </Card>
          ...
          <Card className="p-6 bg-gradient-to-br from-blue-900/20 to-cyan-900/20 border-blue-500/30">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-blue-300 text-sm">{t('market.baseCurrency')}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-xl font-semibold text-white">
                  1 APE = {formatPrice(marketData.apeUsd / marketData.craUsd, 0)} CRA
                </span>
              </div>
              {craData && (
                <div className="flex justify-between items-center">
        ...
        <Card className="p-6 bg-slate-800/50 backdrop-blur-sm border-slate-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="text-center">
              ...
              <p className={`text-xl font-bold ${getPriceChangeColor(craData.price_change_1h)}`}>
                {craData.price_change_1h > 0 ? '+' : ''}{craData.price_change_1h.toFixed(2)}%
              </p>
            </div>
            
            <div className="text-center">
              <p className="text-slate-400 text-sm">{t('market.change24h')}</p>
              <p className={`text-xl font-bold ${getPriceChangeColor(craData.price_change_24h)}`}>
                {craData.price_change_24h > 0 ? '+' : ''}{craData.price_change_24h.toFixed(2)}%
              </p>
            </div>
