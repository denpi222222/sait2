...
import { useCRATokenStat } from "@/hooks/useCRATokenStat";
...
export function TokenAllocationDonut() {
  const { stat, loading } = useCRATokenStat();

  const allocations: Allocation[] = stat
    ? [
        { name: "Treasury", value: Number(stat.treasury), color: COLORS[0] },
        { name: "DEX Liquidity", value: Number(stat.dexLiquidity), color: COLORS[1] },
  ...
  const data = allocations.map((a) => ({ ...a, pct: totalRaw ? (a.value / totalRaw) * 100 : 0 }));

  if (loading && !stat) {
    return (
      <ChartCard title="CRA Token Allocation" icon={<PieChart className="h-5 w-5" />} color="cyan">
        ...
        <div className="relative w-44 h-44">
          <svg viewBox="0 0 100 100" className="w-full h-full rotate-[-90deg]">
            {data.map((item, index) => {
              ...
              const offset = data.slice(0, index).reduce((sum, d) => sum + d.pct / 100, 0)
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-2xl font-bold text-slate-200">CRA</span>
      ...
      <div className="flex flex-col mt-4 space-y-2">
        {data.map((item) => (
          <div key={item.name} className="flex items-center justify-between">
            ...
            <div className="flex items-center">
              <span className="text-sm font-medium text-slate-200 mr-2">{item.pct.toFixed(1)}%</span>
            </div>
          </div>
        ))}
      </div>
