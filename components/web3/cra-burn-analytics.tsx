...
export default function CRABurnAnalytics() {
  ...
  const fetchBurnAnalytics = async () => {
    try {
      ...
      console.error('Burn analytics error:', err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <Card className="p-4 bg-red-900/20 border-red-500/30">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-red-300 text-sm">Total CRA burned</p>
                <p className="text-2xl font-bold text-white">{formatCRA(analytics.totalCRABurned)}</p>
              </div>
          ...
          <Card className="p-4 bg-orange-900/20 border-orange-500/30">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">{formatNumber(analytics.totalBurns)}</p>
              </div>
        ...
        >
          <Card className="p-4 bg-green-900/20 border-green-500/30">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-green-300 text-sm">CRA distributed</p>
                <p className="text-2xl font-bold text-white">{formatCRA(analytics.totalCRADistributed)}</p>
              </div>
          ...
          <Card className="p-4 bg-purple-900/20 border-purple-500/30">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white">{formatNumber(analytics.totalClaims)}</p>
              </div>
        ...
        <Card className="p-6 bg-slate-800/50 backdrop-blur-sm border-slate-700">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="text-blue-400 font-semibold">{formatNumber(analytics.totalActivations)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">In graveyard:</span>
              <span className="text-red-400 font-semibold">{formatNumber(parseInt(analytics.graveyardSize))}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Tokens burned:</span>
              <span className="text-orange-400 font-semibold">{formatCRA(analytics.totalTokensBurned)}</span>
            </div>
          </div>
        </Card>
            ...
            <div className="flex justify-between">
              ...
              <span className="text-green-400 font-semibold">
                {analytics.totalBurns > 0 ? 
                  `${(parseInt(analytics.totalCRABurned) / 1e18 / analytics.totalBurns).toFixed(0)} CRA/NFT` : 
                  'N/A'
                }
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span className="text-yellow-400 font-semibold">
                {analytics.totalClaims > 0 ? 
                  `${(parseInt(analytics.totalCRADistributed) / 1e18 / analytics.totalClaims).toFixed(0)} CRA/claim` : 
                  'N/A'
                }
              </span>
            </div>
          ...
          <div className="space-y-3">
            <div>
              <div className="flex justify-between mb-1">
                ...
                <span className="text-red-400 text-sm">{formatCRA(analytics.totalCRABurned)} CRA</span>
              </div>
              <div className="w-full bg-slate-700 rounded-full h-2">
                <div className="h-2 bg-red-400 rounded-full" style={{ width: '60%' }}></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between mb-1">
                ...
                <span className="text-green-400 text-sm">{formatCRA(analytics.totalCRADistributed)} CRA</span>
              </div>
              <div className="w-full bg-slate-700 rounded-full h-2">
            ...
            {analytics.recentClaims.map((claim, index) => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-white font-semibold">NFT #{claim.tokenId}</p>
                    <p className="text-slate-400 text-sm">by {formatAddress(claim.player)}</p>
                    <div className="mt-2 space-y-1">
                      <p className="text-green-400 text-sm">
                        To player: {formatCRA(claim.playerShare)} CRA
                      </p>
                      <p className="text-red-400 text-sm">
                        Burned: {formatCRA(claim.burnedShare)} CRA
                      </p>
                      <p className="text-blue-400 font-semibold">
                        Total: {formatCRA(claim.totalAmount)} CRA
                      </p>
                    </div>
                  </div>
