...
export default function NFTGraveyardCard({ nft, index, onActionComplete }: NFTGraveyardCardProps) {
  ...
  return (
    <motion.div initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} transition={{delay:index*0.05}} className="scale-[0.9]">
      <Card className="bg-gradient-to-br from-gray-800/80 to-slate-800/80 border border-gray-600/30 hover:border-gray-400/60">
        <CardHeader>
          <div className="aspect-square relative overflow-hidden rounded-lg burned-img">
            {getNFTImage(nft)?(
               <Image src={getNFTImage(nft)} alt={`Cube #${tokenIdDec}`} width={200} height={200} className="w-full h-full object-cover grayscale brightness-75" />
              ):(<div className="w-full h-full bg-gray-700 flex items-center justify-center" />)}
            <div className="absolute inset-0 bg-black/50 flex items-center justify-center"><Skull className="w-10 h-10 text-red-600" /></div>
          </div>
        </CardHeader>
        <CardContent className="text-center space-y-2">
          <p className="text-gray-300 text-sm">Token #{tokenIdDec}</p>
          {gameData && (
            <p className="text-xs text-amber-400">{getRarityLabel(gameData.rarity)}</p>
          )}
          {loading ? (
            <div className="text-xs text-gray-400"><Loader2 className="w-4 h-4 animate-spin inline"/> Loading</div>
          ) : (
            <>
              <p className="text-xs text-gray-400 flex items-center justify-center gap-1">
                {claimReady ? (<><Coins className="w-4 h-4"/> Reward ready</>) : (<><TimerReset className="w-4 h-4"/> {formatDuration(timeLeft)}</>)}
              </p>
              <Button disabled={!claimReady || processing} onClick={handleClaim} className="w-full bg-gradient-to-r from-slate-700 to-gray-700 hover:from-slate-600 hover:to-gray-600 text-white">
                {processing? (<Loader2 className="w-4 h-4 animate-spin" />): claimReady? 'Claim' : 'Waiting'}
              </Button>
            </>
          )}
        </CardContent>
