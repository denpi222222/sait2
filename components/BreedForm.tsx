...
export function BreedForm() {
  ...
  const { has: graveyardHas, loading: loadingGraveyard } = useGraveyardAvailability()
    ...
    <Card className="mx-auto max-w-2xl mt-6">
      ...
      <CardContent>
        {loadingGraveyard ? (
          ...
          <p className="text-center text-red-400 mb-4">Graveyard is empty. Bridging temporarily unavailable.</p>
        ) : isLoadingNFTs ? (
          ...
          <div>
            <p className="mb-4 text-sm text-slate-300">Select two cubes to send for breeding. One random cube will jump out from the graveyard!</p>
            <div className="grid grid-cols-2 gap-3 mb-6 max-h-80 overflow-y-auto pr-2">
              {nfts.map((nft) => {
                const selected = nft.tokenId === parent1 || nft.tokenId === parent2
                return (
                  <button
                    type="button"
                    key={nft.id}
                    onClick={() => {
                      ...
                      else if (parent2 === null && nft.tokenId !== parent1) setParent2(nft.tokenId)
                  ...
                  >
                    ...
                    <span className="block mt-1 text-center">#{nft.tokenId}</span>
                  </button>
                )
              })}
            </div>
            <div className="space-y-2">
              ...
              {typeof breedCost === "bigint" && (
                <p>Breed cost: {parseFloat(formatEther(breedCost)).toFixed(2)} CRA</p>
              )}
              {breedError && <p className="text-red-500 text-sm">{breedError.message}</p>}
              <Button disabled={!isApeChain || !isReady || isBreeding} onClick={handleBreed} className="w-full">
