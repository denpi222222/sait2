...
function noise2D(x: number, z: number): number {
  return Math.sin(x * 0.01) * Math.cos(z * 0.01) * 
         Math.sin(x * 0.001) * Math.cos(z * 0.001) * 32 + 16
}
...
class Chunk {
          ...
          if (blockType === BlockType.GRASS && Math.random() < 0.01) {
            for (let treeY = 0; treeY < 5; treeY++) {
              if (y + treeY < Chunk.SIZE) {
                this.setBlock(x, y + treeY, z, treeY < 3 ? BlockType.WOOD : BlockType.LEAVES)
              }
            }
          }
  ...
  setBlock(x: number, y: number, z: number, type: BlockType) {
    if (x < 0 || x >= Chunk.SIZE || y < 0 || y >= Chunk.SIZE || z < 0 || z >= Chunk.SIZE) return
      ...
      for (let y = 0; y < Chunk.SIZE; y++) {
        for (let z = 0; z < Chunk.SIZE; z++) {
          ...
          for (let face = 0; face < 6; face++) {
            if (this.shouldRenderFace(x, y, z, directions[face])) {
              ...
              for (let tri = 0; tri < 2; tri++) {
                ...
                for (let i = 0; i < 3; i++) {
                  ...
                  normals.push(directions[face][0], directions[face][1], directions[face][2])
...
class VoxelWorld {
  ...
  loadChunk(x: number, y: number, z: number): Chunk {
    ...
    if (!chunk) {
      ...
      console.log(`Loaded chunk ${x},${y},${z}`)
    }
  ...
  unloadChunk(x: number, y: number, z: number) {
    ...
    if (chunk && chunk.mesh) {
      ...
      console.log(`Unloaded chunk ${x},${y},${z}`)
    }
  }
...
export default function VoxelEngine({ onExit }: VoxelEngineProps) {
  ...
  useEffect(() => {
    ...
    console.log(`🎮 Player spawned at ground level: ${groundY}`)
    ...
    function updatePlayer() {
      ...
      const speed = MOVE_SPEED * (movement.run ? RUN_MULTIPLIER : 1) * deltaTime
    ...
    loader.load('/images/Cubie_Detective_1_texture.glb', (gltf) => {
      const npc = gltf.scene
      npc.scale.set(1, 1, 1)
      npc.position.set(2, 50, 0)
      scene.add(npc)
      
      console.log('🕵️ Cubie Detective loaded successfully!')
    }, undefined, (err) => {
      ...
      console.log('🎲 Using fallback cube detective')
    })
  ...
  return (
    <div className="relative w-full h-screen">
      ...
      <div className="absolute top-4 left-4 bg-black/70 text-green-400 p-3 rounded font-mono text-sm">
        <div>🕵️ Cubie Detective: X: {playerPos.x}, Y: {playerPos.y}, Z: {playerPos.z}</div>
