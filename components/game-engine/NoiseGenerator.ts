...
export class NoiseGenerator {
  ...
  fractalNoise2D(x: number, y: number, octaves: number = 4, persistence: number = 0.5): number {
  ...
  warpedNoise2D(x: number, y: number, warpStrength: number = 0.1): number {
    ...
    const warpY = this.fractalNoise2D(x * 0.1 + 100, y * 0.1 + 100) * warpStrength
...
export class BiomeGenerator {
  ...
  getBiome(x: number, z: number): BiomeType {
    const temperature = this.temperatureNoise.fractalNoise2D(x * 0.005, z * 0.005)
