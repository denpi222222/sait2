...
export const CubiePlayer: React.FC<CubiePlayerProps> = ({
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        console.log('🕵️ Cubie Detective loaded!', gltf)
        ...
        if (gltf.animations && gltf.animations.length > 0) {
          ...
          console.log('🎪 No animations found, creating procedural')
          createProceduralAnimation(model)
        }
      },
      (progress) => {
        console.log('📦 Cubie Detective loading:', Math.round((progress.loaded / progress.total) * 100) + '%')
      },
      (error) => {
        console.error('❌ Cubie Detective load error:', error)
  ...
  const createFallbackCube = (parent: THREE.Group) => {
    console.log('🎲 Creating simple detective cube')
    // Main body
