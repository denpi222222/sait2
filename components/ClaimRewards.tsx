...
const ClaimableNFTCard = ({ nft, tokenId, onClaim, isLoading }: ClaimableNFTCardProps) => {
  ...
  const { data: currentOwner } = useReadContract({
    ...
    abi: [{ inputs: [{internalType:'uint256', name:'tokenId', type:'uint256'}], name:'ownerOf', outputs:[{internalType:'address', name:'owner', type:'address'}], stateMutability:'view', type:'function'}] as const,
  ...
  if (!burnRecord || burnRecord.claimed || parseFloat(burnRecord.lockedAmountFormatted) === 0) {
    return null; // Don't show if no burn record or already claimed
  }

  const isRevivedByOther = currentOwner &&
    currentOwner.toLowerCase() !== GAME_CONTRACT_ADDRESS.toLowerCase() &&
    wallet && currentOwner.toLowerCase() !== wallet.toLowerCase();

  const getWaitPeriodColor = (period: number) => {
    switch (period) {
      case 0: return 'bg-red-500'; // 12h
      case 1: return 'bg-orange-500'; // 24h
      case 2: return 'bg-yellow-500'; // 48h
  ...
  const getRewardPercentage = (period: number) => {
    switch (period) {
      case 0: return 50; // 12h
      case 1: return 60; // 24h
      case 2: return 70; // 48h
      default: return 50;
    }
  };

  const rewardPercentage = getRewardPercentage(burnRecord.waitPeriod);
  const estimatedReward = parseFloat(burnRecord.lockedAmountFormatted) * (rewardPercentage / 100);
      ...
      <Card className="border-orange-500/30 bg-slate-900/50 backdrop-blur-sm hover:border-orange-500/50 transition-colors">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Badge className={`${getWaitPeriodColor(burnRecord.waitPeriod)} text-white`}>
        ...
        <CardContent className="space-y-4">
            ...
            <div className="flex-1 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-green-300 font-mono font-bold">
                  {estimatedReward.toFixed(2)} CRA ({rewardPercentage}%)
                </span>
              </div>
                ...
                <div className="flex items-center gap-1">
                  {burnRecord.canClaim ? (
                    <>
                      ...
                      <span className="text-green-400 font-medium">{t('status.readyToClaim', 'Ready to Claim')}</span>
                    </>
                  ) : (
                    <>
                      <Clock className="w-4 h-4 text-yellow-400" />
                      <span className="text-yellow-400">{burnRecord.timeLeftFormatted}</span>
                    </>
                  )}
                </div>
              </div>
          ...
          >
            ...
            {burnRecord.canClaim 
              ? `Claim ${estimatedReward.toFixed(2)} CRA` 
...
export const ClaimRewards = () => {
  const { rewards, loading: isLoadingNfts, error: nftsError, refreshing, refresh } = usePendingBurnRewards();
  ...
  if (isLoadingNfts && rewards.length===0) {
    return (
      <div className="flex justify-center items-center py-12">
        ...
        />
        <span className="ml-3 text-green-300">Loading claimable rewards...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-slate-400">
          Collect rewards from your burned NFTs
        </p>
        <div className="mt-4">
      ...
      {rewards.length === 0 && (
        ...
        >
          ...
          <h3 className="text-xl font-semibold text-green-300 mb-2">No Rewards to Claim</h3>
          <p className="text-slate-400">Burn some NFTs first to earn rewards!</p>
        </motion.div>
      )}
        ...
        >
          <div className="flex justify-center items-center gap-2">
            ...
            <span className="text-blue-300">
              {isWritePending ? t('sections.claim.waitingForWallet', 'Waiting for wallet confirmation...') : t('sections.claim.processingClaim', 'Processing claim...')}
            </span>
          </div>
          {txHash && (
      ...
      {isTxSuccess && (
        ...
        >
          <div className="text-green-400 text-lg">🎉 Rewards Claimed Successfully!</div>
          <p className="text-sm text-slate-400 mt-1">Your CRA rewards have been transferred to your wallet.</p>
        </motion.div>
      )}
