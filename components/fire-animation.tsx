...
export function FireAnimation({
  ...
  useEffect(() => {
    ...
    interface P { x:number; y:number; r:number; vy:number; hue:number }
    const flames:P[]=[]
    const sparks:P[]=[]

    const mkFlame=():P=>({x:rand(0,w),y:h+rand(0,h*0.3),r:rand(30,80),vy:rand(-2.5,-1.2),hue:rand(20,40)})
    const mkSpark=():P=>({x:rand(0,w),y:h+rand(0,60),r:rand(2,4),vy:rand(-5,-3),hue:rand(10,20)})
    ...
    const draw=()=>{
      ...
      flames.forEach(p=>{
        ...
        ctxBig.fillStyle=g; ctxBig.beginPath(); ctxBig.arc(p.x,p.y,p.r,0,Math.PI*2); ctxBig.fill()
      })
  ...
  return (
    <div className={`pointer-events-none fixed inset-x-0 bottom-0 ${className}`}>
