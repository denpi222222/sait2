...
export function CrazyCubeInteraction() {
  ...
  return (
    <div style={{ border: '1px solid #ccc', padding: '20px', margin: '20px 0' }}>
      ...
      <p><strong>Your address:</strong> {accountAddress}</p>
      <p><strong>Network:</strong> {chain?.name} (ID: {chain?.id})</p>

      <div style={{ marginBottom: '15px' }}>
        <h3>Contract information:</h3>
        {isLoadingTotalSupply && <p>Loading total NFT supply...</p>}
        {totalSupply !== undefined && totalSupply !== null && <p><strong>Total NFT supply:</strong> {totalSupply.toString()}</p>}
        {errorTotalSupply && <p style={{ color: 'red' }}>Error loading totalSupply: {errorTotalSupply.message}</p>}
        <button onClick={() => refetchTotalSupply()} disabled={isLoadingTotalSupply}>
      ...
      <div style={{ marginBottom: '15px' }}>
        ...
        {balanceOf !== undefined && balanceOf !== null && <p><strong>Your NFT balance:</strong> {balanceOf.toString()}</p>}
        {errorBalanceOf && <p style={{ color: 'red' }}>Error loading balanceOf: {errorBalanceOf.message}</p>}
        <button onClick={() => refetchBalanceOf()} disabled={isLoadingBalanceOf}>
      ...
      <div style={{ marginBottom: '15px' }}>
        ...
        />
        <button onClick={handleBurn} disabled={isSubmittingTx || isConfirmingTx}>
          {isSubmittingTx ? 'Submitting...' : isConfirmingTx ? 'Confirming...' : 'Burn NFT'}
        </button>
      </div>
      ...
      <div style={{ marginBottom: '15px' }}>
        ...
        <input
          ...
          style={{ marginRight: '10px', padding: '5px' }}
        />
        <button onClick={handleActivate} disabled={isSubmittingTx || isConfirmingTx}>
          {isSubmittingTx ? 'Submitting...' : isConfirmingTx ? 'Confirming...' : 'Activate NFT'}
        </button>
      </div>

      {lastTxHash && <p><strong>Last transaction hash:</strong> <a href={`https://apescan.io/tx/${lastTxHash}`} target="_blank" rel="noopener noreferrer">{lastTxHash}</a></p>}
      {writeContractError && <p style={{ color: 'red' }}><strong>Transaction send error:</strong> {writeContractError.message}</p>}
      {txConfirmationError && <p style={{ color: 'red' }}><strong>Transaction confirmation error:</strong> {txConfirmationError.message}</p>}
