...
export const PerformanceInfo = () => {
  const { isLiteMode, isMobile, isWeakDevice, performanceInfo } = usePerformanceContext();
  ...
  const getConnectionIcon = () => {
    if (connectionSpeed === 'slow') return <WifiOff className="w-4 h-4 text-red-400" />;
    if (connectionSpeed === 'fast') return <Wifi className="w-4 h-4 text-green-400" />;
        ...
        {isLiteMode && (
          <div className="mt-4 p-2 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-md">
            <p className="text-xs text-orange-700 dark:text-orange-300">
              Lite mode automatically enabled for better performance on this device.
            </p>
          </div>
        )}
      </CardContent>
