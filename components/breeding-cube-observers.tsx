...
export default function CubeObservers({ selectionCount, phase }: CubeObserversProps) {
      ...
      <AnimatePresence>
        {observers.map((c, idx) => {
          ...
          const start: TargetAndTransition =
            c.dir === "left"   ? { x: -200, y: `${c.pos}vh`, opacity: 0 } :
            c.dir === "right"  ? { x:  200, y: `${c.pos}vh`, opacity: 0 } :
            c.dir === "top"    ? { y: -200, x: `${c.pos}vw`, opacity: 0 } :
          ...
          return (
            ...
            >
              {/* Walker */}
              <img src={c.img} alt="cube observer" className="w-48 h-48 drop-shadow-xl" />
