...
import { useReadContract, useAccount, useConnect, usePublicClient } from 'wagmi'
...
export function UserNFTsPreview() {
  ...
  return (
    <Card className="w-full bg-slate-900/50 border-cyan-500/30">
      ...
      <CardContent>        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 justify-center">
          {displayNfts.map((nft, idx) => {
            const keyVal = getTokenIdAsDecimal(nft as any) || `idx-${idx}`
            return (
              <NFTCard
                key={keyVal}
                nft={nft}
...
function NFTCard({ nft, pingInterval, breedCooldown }: NFTCardProps) {
  ...
  const currentStars = nftInfo ? nftInfo.dynamic.currentStars : (nft.stars ?? 0)
  const lockedCRA = nftInfo ? (() => {
    try {
      ...
      console.error('Error formatting CRA:', error)
      return 0
    }
  })() : 0
  ...
  const pingReady = pingInterval != null ? nowSec > lastPing + pingInterval : false
  const breedReady = breedCooldown != null ? nowSec > lastBreed + breedCooldown : false
  const burnable = lockedCRA > 0 && !(nftInfo ? nftInfo.dynamic.isInGraveyard : false)
        ...
        {(() => {
          return (
            <div className="absolute top-0.5 right-0.5">
              <Badge className={`${rarityColorClass} text-white text-xs px-1 py-0`}>
                {rarityLabel}
              </Badge>
            </div>
          )
      ...
      {/* NFT Info */}
        ...
        <div className="flex items-center justify-between">
          ...
          {stateLoading ? (
            ...
            <span className="text-yellow-400 font-mono">{'⭐'.repeat(Math.max(1, currentStars))}</span>
          )}
        </div>
        <div className="flex items-center justify-between">
          <span className="text-slate-400">Rarity:</span>
          <span className={`${rarityColorClass} bg-opacity-30 px-1 rounded text-white text-xs`}>{rarityLabel}</span>
        </div>
        <div className="flex items-center justify-between">
          ...
          {stateLoading ? (
            ...
            <span className="text-green-400">
              {lockedCRA >= 1000000 
                ? `${(lockedCRA / 1000000).toFixed(1)}M` 
                : lockedCRA >= 1000 
                  ? `${(lockedCRA / 1000).toFixed(1)}K` 
                  : lockedCRA.toFixed(2)
              }
            </span>
          )}
        ...
        <div className="flex items-center justify-between">
          ...
          <span className={pingReady ? 'text-green-400' : 'text-orange-400'}>
            {pingReady ? t('status.ready', 'Ready') : `⏳ ${pingInterval ? formatDuration(lastPing + pingInterval - nowSec) : ''}`}
          </span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-slate-400">Breed:</span>
          <span className={breedReady ? 'text-green-400' : 'text-orange-400'}>
            {breedReady ? t('status.ready', 'Ready') : `⏳ ${breedCooldown ? formatDuration(lastBreed + breedCooldown - nowSec) : ''}`}
          </span>
        </div>
        <div className="flex items-center justify-between">
