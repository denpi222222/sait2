...
export function IpfsImage({ 
  ...
  const handleError = () => {
    ...
    if (src.includes('d35a2j13p9i4c9.cloudfront.net') || src.includes('cloudflare-ipfs.com')) {
      // Extract IPFS hash from known patterns
      const ipfsMatch = src.match(/ipfs\/([^\/]+)\/(.+)/) || src.match(/QmdRAv2R2MNWEfT3kpostz13qVrDdD2j62jW2i2sra2aA1\/(.+)/)
