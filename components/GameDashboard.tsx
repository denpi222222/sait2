...
const StatCard = ({ title, value, subtitle, icon, color, trend }: StatCardProps) => {
  return (
    ...
    >
      <Card className={`border-${color}-500/30 bg-slate-900/50 backdrop-blur-sm hover:border-${color}-500/50 transition-colors`}>
    ...
    return (
      <div className="text-center py-12">
        ...
        >
          ...
          <h3 className="text-xl font-semibold text-slate-300 mb-2">Connect Your Wallet</h3>
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex flex-wrap justify-center gap-6 text-center">
          <div>
            <div className="text-lg font-bold text-cyan-300">{parseFloat(craBalance).toFixed(2)} CRA</div>
        ...
        <Card className="border-slate-500/30 bg-slate-900/50 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="space-y-3">
              {[
                { action: "NFT Burned", details: "Token #1543 → 12h wait", time: "2 min ago", type: "burn" },
                { action: "Breeding Success", details: "Token #720 + #947 → New NFT", time: "15 min ago", type: "breed" },
                { action: "Rewards Claimed", details: "150.5 CRA claimed", time: "1h ago", type: "claim" },
                { action: "NFT Pinged", details: "Token #1346 earned CRA", time: "2h ago", type: "ping" }
              ].map((activity, index) => (
                <motion.div
                  key={index}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm text-slate-300">{activity.details}</div>
                    </div>
                  </div>
      ...
      >
        <Card className="border-slate-500/30 bg-slate-900/50 backdrop-blur-sm">
          <CardHeader>
            <CardTitle className="text-lg font-semibold text-slate-300">Game Rules</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <h4 className="font-semibold text-orange-300 mb-2">🏃 Ping System</h4>
                <ul className="text-slate-400 space-y-1">
                  <li>• Accumulate CRA every 3 minutes</li>
                  <li>• Reward depends on NFT rarity</li>
                  <li>• Must be activated to ping</li>
                </ul>
              </div>
              <div>
                <h4 className="font-semibold text-red-300 mb-2">🔥 Burn System</h4>
                <ul className="text-slate-400 space-y-1">
                  <li>• Burn NFT for CRA rewards</li>
                  <li>• 5% fee from locked CRA</li>
                  <li>• Choose 12h/24h/48h wait times</li>
                </ul>
              </div>
              <div>
                <h4 className="font-semibold text-pink-300 mb-2">💕 Breeding System</h4>
                <ul className="text-slate-400 space-y-1">
                  <li>• Combine 2 NFTs, lose 1 star each</li>
                  <li>• Cost: {breedCost} CRA</li>
                  <li>• Resurrects NFT from graveyard</li>
                </ul>
              </div>
              <div>
                <h4 className="font-semibold text-green-300 mb-2">💰 Reward System</h4>
                <ul className="text-slate-400 space-y-1">
