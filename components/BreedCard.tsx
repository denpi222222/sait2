...
export default function BreedCard({ nft, index, selected, selectedOrder, disableSelect, onSelect }: BreedCardProps) {
  ...
  const finalDisabled = disableSelect || noStars || hasActiveCooldown
  ...
  return (
    ...
    >
      ...
      <div className={`p-0.5 rounded-lg bg-gradient-to-br from-pink-600/60 to-purple-600/60 ${finalDisabled ? 'grayscale opacity-40' : ''} ${ringClass}`}>
        <div className="relative rounded-lg overflow-hidden">
          <UnifiedNftCard
            imageSrc={resolveImageSrc('image' in nft && nft.image ? nft.image : getNFTImage(nft as AlchemyNFT))}
            tokenId={tokenIdDisplay}
            title={'name' in nft && nft.name ? nft.name : ('title' in nft && (nft as any).title) ? (nft as any).title : `Cube #${tokenIdDisplay}`}
            rarityLabel={rarityLabel ?? ""}
            rarityColorClass={rarityColorClass ?? ""}
            widgets={widgets}
            highlight={!!selected}
            delay={0}
            onClick={() => { if(!finalDisabled && tokenIdNum!==null) onSelect?.(tokenIdNum) }}
          />
            ...
            >
              <div className="relative">
                ...
                <div className="relative bg-gradient-to-r from-red-600 to-orange-600 text-white text-3xl font-black px-3 py-1.5 rounded-xl border-3 border-yellow-400 shadow-2xl">
                  ...
                  <span className="relative drop-shadow-2xl" style={{ textShadow: '0 0 10px #ff0000, 0 0 20px #ff0000, 0 0 30px #ff0000' }}>
                    -1 ⭐
                  </span>
                </div>
                {/* Warning text below */}
          ...
          {!selected && !finalDisabled && (
            <button
              onClick={() => { if(tokenIdNum!==null) onSelect?.(tokenIdNum) }}
          ...
          {hasActiveCooldown && gameInfo && (
            <div className="absolute bottom-3 left-2 right-2 flex items-center justify-center gap-2 text-lg font-bold text-orange-300 bg-gradient-to-r from-orange-700/80 to-pink-700/80 rounded-xl py-1 px-2 shadow-lg pointer-events-none border-2 border-orange-400/40">
              <span className="flex items-center"><svg xmlns='http://www.w3.org/2000/svg' className='w-5 h-5 mr-1 text-orange-200' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M12 8v4l3 2m6-2a10 10 0 11-20 0 10 10 0 0120 0z' /></svg></span>
              <span>{t('sections.breed.cooldown', 'Cooldown:')}</span>
              <span className="text-white drop-shadow-lg">{formatDuration(gameInfo.breedCooldown)}</span>
            </div>
          )}
        </div>
      </div>
