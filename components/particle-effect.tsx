...
export function ParticleEffect({
  ...
  const createParticle = useCallback((width: number, height: number, colors: string[], size: number): Particle => {
    ...
    const speed = Math.random() * (isMobileSize ? maxVelocity * 0.7 : maxVelocity) + 0.5

    // Ensure size is always positive
    const particleSize = Math.max(2, Math.random() * (isMobileSize ? size * 0.8 : size) + 2)
  ...
  const initialParticles = useMemo(() => {
    if (!isClient || !enabled || prefersReduced || dimensions.width === 0) return []

    // Reduce particle count for better performance
    const actualCount = dimensions.width < 768 ? Math.floor(count * 0.5) : Math.floor(count * 0.7)
    return Array.from({ length: actualCount }, () => createParticle(dimensions.width, dimensions.height, colors, size))
    ...
    const animate = (time: number) => {
      ...
      if (shouldRender) {
        const updatedParticles = particlesRef.current.map((particle) => {
          // Update position
          const newX = particle.x + particle.velocity.x * speed * (deltaTime / 16)
          const newY = particle.y + particle.velocity.y * speed * (deltaTime / 16)
