...
export function AshEffect({
  ...
  // Create particle
  const createParticle = useCallback((w: number, h: number): AshParticle => {
    ...
    const vx = verticalOnly ? (Math.random() - 0.5) * 10 : 10 + Math.random() * 20 // slight sway if vertical only
  ...
  const initialParticles = useMemo(() => {
    ...
    return Array.from({ length: count }, () => createParticle(dimensions.width, dimensions.height))
