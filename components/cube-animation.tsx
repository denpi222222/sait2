...
const SpeechBubble = React.memo(function SpeechBubble({ text, className = "" }: { text: string; className?: string }) {
  return (
...
export function CubeAnimation() {
  ...
  const [cubesLeft, setCubesLeft] = useState<boolean[]>([true, true, true, true, true])
  ...
  const [waitingForMusicPhraseIndex, setWaitingForMusicPhraseIndex] = useState(0)
  const [showWaitingForMusicPhrase, setShowWaitingForMusicPhrase] = useState(false)
  ...
  const getUniqueRandomCubeIndices = useCallback((count: number, excludeIndex?: number) => {
    const indices: number[] = []
    const availableIndices = [...Array(cubeImages.length).keys()].filter((i) => i !== excludeIndex)
  ...
  const startWaitingForMusicPhrasesCycle = useCallback(() => {
    ...
    const showNextWaitingPhrase = () => {
      ...
      const waitingPhrases = t("cubeAnimation.waitingForMusicPhrases", { returnObjects: true }) as string[]
  ...
  const startPartyPhrasesCycle = useCallback(() => {
    ...
    const showNextPhrase = () => {
      ...
      const partyPhrases = t("cubeAnimation.partyPhrases", { returnObjects: true }) as string[]
  ...
  const speechBubbles = t("cubeAnimation.speechBubbles", { returnObjects: true }) as string[]
  const partyBubble = t("cubeAnimation.partyBubble")
  const whereIsEveryone = t("cubeAnimation.whereIsEveryone")
  const partyPhrases = t("cubeAnimation.partyPhrases", { returnObjects: true }) as string[]
  const waitingForMusicPhrases = t("cubeAnimation.waitingForMusicPhrases", { returnObjects: true }) as string[]
      ...
      {showMusicButton && (
        ...
        >
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="bg-black/80 border-cyan-500/50 text-cyan-300">
              ...
              <DropdownMenuItem onClick={toggleMusic} className="cursor-pointer border-t border-cyan-500/30 mt-1 pt-1">
                {isMusicPlaying ? (
                  <>
                    <VolumeX className="mr-2 h-4 w-4" />
              ...
              <motion.div
                ...
                transition={
                  animationPhase >= 2
                    ...
                    : { duration: 1, repeat: animationPhase >= 1 ? Number.POSITIVE_INFINITY : 0 }
                }
          ...
          >
            <motion.div
              className={`absolute inset-0 rounded-full ${
                partyMode ? getBackgroundForTheme(currentTrack?.theme || "party") : "bg-blue-500/30"
              } blur-xl`}
              animate={{
                scale: [1, 1.2, 1],
                opacity: [0.5, 0.8, 0.5],
        ...
        <>
          ...
          {Array.from({ length: isMobile ? 10 : 20 }).map((_, i) => {
            ...
            if (currentTrack?.theme === "retro") {
              particleColor = i % 3 === 0 ? "bg-purple-500" : i % 3 === 1 ? "bg-indigo-500" : "bg-violet-500"
            } else if (currentTrack?.theme === "chill") {
              particleColor = i % 3 === 0 ? "bg-green-500" : i % 3 === 1 ? "bg-emerald-500" : "bg-teal-500"
            } else if (currentTrack?.theme === "dance") {
              particleColor = i % 3 === 0 ? "bg-pink-500" : i % 3 === 1 ? "bg-rose-500" : "bg-red-500"
            } else {
              particleColor =
                i % 5 === 0
                  ? "bg-pink-500"
