...
import { Flame, Star, Zap, Clock, Loader2, SatelliteDish, Heart } from "lucide-react"
import Image from "next/image"

import { AlchemyNFT, getNFTImage, getTokenIdAsDecimal } from "@/hooks/useUserNFTs"
...
export default function NFTBurnCard({ nft, index, onActionComplete }: NFTBurnCardProps) {
  ...
  const [step, setStep] = useState<'idle' | 'approvingCRA' | 'approvingNFT' | 'burning'>('idle')
  const [waitHours, setWaitHours] = useState<12 | 24 | 48>(12)
  const [burnSplit, setBurnSplit] = useState<{playerBps: number; poolBps: number; burnBps: number}>({playerBps: 0, poolBps:0, burnBps:0})
  ...
  const calcFee = (): string => {
    ...
    const lockedWei = parseEther(gameData.lockedCRA)
  ...
  const calcShares = () => {
    ...
    const totalWei = parseEther(gameData.lockedCRA)
    ...
    return {user: formatEther(userWei), pool: formatEther(poolWei), burn: formatEther(burnWei)}
  }
  ...
  const handleBurn = async () => {
    ...
    const confirm = window.confirm(
      `🔥 Burn NFT #${tokenIdDecimal}?\n\nLocked CRA: ${gameData.lockedCRA}\nFee (${burnFeeBps / 100}%): ${fee} CRA` +
    ...
    try {
      ...
      await approveCRA(fee)

      setStep('approvingNFT')
      toast({ title: 'Approving NFT', description: `Token #${tokenIdDecimal}` })
      ...
      toast({ title: 'NFT burned', description: `Sent to graveyard. Claim after ${waitHours}h` })
  ...
  const pingReady = gameData ? (gameData.lastPingTime === 0 || nowSec > gameData.lastPingTime + pingInterval) : false
  const pingTimeLeft = gameData ? Math.max(0, (gameData.lastPingTime + pingInterval) - nowSec) : 0

  const breedReady = gameData ? (gameData.currentStars > 0 && nowSec > gameData.lastBreedTime + breedCooldown) : false
  const breedTimeLeft = gameData ? Math.max(0, (gameData.lastBreedTime + breedCooldown) - nowSec) : 0

  const noCRA = gameData ? Number(gameData.lockedCRA) === 0 : false
  ...
  return (
    ...
    >
      ...
      {step === 'burning' && (
        <BurnEffect isActive onComplete={() => { /* overlay hides itself after animation */ }} />
      )}
      <Card
        className={`bg-gradient-to-br from-slate-900/80 to-slate-800/80 border-2 transition-all duration-300 hover:scale-105 ${
          selected ? 'border-orange-500 shadow-lg shadow-orange-500/25' : 'border-slate-700 hover:border-orange-500/50'
        } ${noCRA ? 'opacity-40 grayscale pointer-events-none' : ''}`}
      >
        <CardHeader className="pb-3">
          <div className="relative">
            ...
            {gameData && (
              <Badge className={`absolute top-1.5 right-1.5 ${rarityInfo(gameData.rarity).color} text-white text-xs`}>
            ...
            {gameData && gameData.isInGraveyard && (
              <div className="absolute bottom-2 left-2 bg-red-500/80 text-white text-xs px-2 py-1 rounded">💀 Graveyard</div>
            )}
          </div>
        </CardHeader>
        <CardContent className="pt-0">
          <div className="text-center mb-4">
            <h3 className="font-semibold text-white text-lg mb-1">{nft.title || `CrazyCube #${tokenIdDecimal}`}</h3>
            ...
            {gameData ? (
              <div className="text-xs text-gray-500 mt-2 space-y-1">
                ...
                {Number(gameData.lockedCRA) > 0 ? (
                  <p className="text-lg font-extrabold text-orange-400">💰 CRA: {gameData.lockedCRA}</p>
                ) : (
                  <p className="text-sm text-gray-500">No CRA locked</p>
                )}
                {/* Ping & Breed status condensed */}
                <p>
                  <SatelliteDish className="inline w-3 h-3 mr-1" />{pingReady ? t('status.ready', 'Ready') + ' ✓' : `Ping ${formatDuration(pingTimeLeft)}`}
                </p>
                <p>
                  <Heart className="inline w-3 h-3 mr-1" />{breedReady ? t('status.ready', 'Ready') + ' ✓' : (gameData.currentStars === 0 ? t('status.noStarsLeft', 'No Stars') : `Breed ${formatDuration(breedTimeLeft)}`)}
                </p>
                <p>{gameData.isActivated ? '✅ Activated' : '❌ Not Activated'}</p>
              </div>
            ) : (
              <div className="text-xs text-gray-500 mt-2">
                <Loader2 className="w-4 h-4 animate-spin inline-block mr-1" /> Loading...
              </div>
            )}
          </div>
                ...
                >
                  {isProcessing ? (
                    <>
                      ...
                      {step === 'approvingCRA' && 'Approving CRA...'}
            ...
            <div className="flex justify-center gap-2 mb-2">
              {[12,24,48].map(h => (
                <Button key={h}
                        ...
                        className={waitHours===h? 'px-3 py-1' : 'px-3 py-1 border-orange-500/30'}
          ...
          {selected && gameData && (
            <div className="text-xs text-gray-400 mb-2 space-y-1">
              {(()=>{const s=calcShares(); return (
                <>
                  <p>👤 You will get: <span className="text-orange-300">{s.user} CRA</span> ({burnSplit.playerBps/100}%)</p>
                  <p>🏦 To Pool: {s.pool} CRA ({burnSplit.poolBps/100}%)</p>
                  <p>🔥 Burned: {s.burn} CRA ({burnSplit.burnBps/100}%)</p>
                </>
              )})()}
            </div>
          )}
