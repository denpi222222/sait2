...
export function ConfirmProvider({ children }: { children: ReactNode }) {
  ...
  const handleClose = (result: boolean) => {
    setOpen(false)
    if(result){
      toast({ title: opts?.title || 'Confirmed', description: '👍 Action confirmed', duration: 4000 })
    }
    resolver(result)
  }
        ...
        <AlertDialog open={open} onOpenChange={(v)=>!v && handleClose(false)}>
          <AlertDialogPortal>
            ...
            <AlertDialogContent className="bg-gradient-to-br from-slate-800 to-slate-900 border-2 border-pink-500/40 shadow-xl max-w-sm">
              ...
              <AlertDialogFooter>
                <AlertDialogCancel onClick={()=>handleClose(false)} className="border-pink-400/50 text-pink-300 hover:bg-pink-500/10">
                  {opts.cancelText || 'Cancel'}
                </AlertDialogCancel>
                <AlertDialogAction onClick={()=>handleClose(true)} className="bg-pink-600 hover:bg-pink-500 text-white">
