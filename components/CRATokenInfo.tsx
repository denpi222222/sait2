...
import { TrendingUp, TrendingDown, DollarSign, Coins, Flame, RefreshCw, ExternalLink, AlertCircle, BarChart3 } from 'lucide-react';

// CRA Token API
const CRA_API = '/api/cra-token';
const CRA_TOKEN_ADDRESS = '0x0A5b48dB89Bf94466464DE3e70F9c86aa27b9495';

// Public Viem client (read-only)
const publicClient = createPublicClient({ transport: http('https://rpc.apechain.com') });
...
const BURN_CACHE_KEY = 'cra_burned_cache';           // amount burned (wei) + ts
...
export default function CRATokenInfo() {
  ...
  const fetchCRAData = async () => {
    ...
    try {
      console.log('🪙 Fetching CRA token data...');
      
      // 1. Request price data from GeckoTerminal API
      let priceData = null;
      try {
        console.log('💰 Fetching price data from GeckoTerminal...');
        const priceRes = await fetch(CRA_API);
        ...
        if (priceResult.success) {
          ...
          console.warn('⚠️ Price API error:', priceResult.error);
        }
        // DexScreener fallback if GeckoTerminal failed or returned null
        if (!priceData) {
          try {
            console.log('🔄 Fallback to DexScreener...');
            const dsRes = await fetch('https://api.dexscreener.com/latest/dex/pairs/apechain/0xab6242f2fbb14caefe9447126704a17947842c6f');
            ...
            if (dsJson?.pair) {
              ...
              console.log('✅ DexScreener price:', priceData);
            }
          } catch (dsErr) {
            console.warn('⚠️ DexScreener fallback error', dsErr);
          }
        } 
      } catch (priceError) {
...
;
        ...
        console.warn('⚠️ Subgraph error:', subgraphError);
      }

      // 2.2 Get burned CRA (from cache or on-chain)
      let burnedWei: bigint = 0n;
      try {
        const cachedRaw = typeof window !== 'undefined' ? localStorage.getItem(BURN_CACHE_KEY) : null;
        if (cachedRaw) {
        ...
        if (burnedWei === 0n) {
          ...
          if (cachedInitRaw) {
            ...
            if (totalSupplyWei > initialSupplyWei) {
              ...
              if (typeof window !== 'undefined') {
                localStorage.setItem(INITIAL_SUPPLY_KEY, initialSupplyWei.toString());
              }
            }
          } else {
            initialSupplyWei = totalSupplyWei;
          ...
          burnedWei = initialSupplyWei > totalSupplyWei ? initialSupplyWei - totalSupplyWei : 0n;
          if (burnedWei < 0n) burnedWei = 0n;
          if (typeof window !== 'undefined') {
            localStorage.setItem(BURN_CACHE_KEY, JSON.stringify({ ts: Date.now(), value: burnedWei.toString() }));
          }
        }
      } catch (burnErr) {
        console.warn('⚠️ Burn fetch error', burnErr);
      }
      ...
      const avgBurnAmount = recentBurns.length > 0 
        ? recentBurns.reduce((sum: number, burn: any) => sum + parseFloat(burn.amount || '0'), 0) / recentBurns.length
        : 0;
      ...
      setError(error.message || 'Failed to fetch CRA data');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Card className="p-6 bg-gradient-to-r from-orange-900/20 to-yellow-900/20 border-orange-500/30">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">CRA Token</h2>
              <p className="text-slate-300 text-sm">Crazy Adventure Token</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-slate-800/50 p-4 rounded-lg">
            ...
            <div className="text-sm text-slate-400">
              Avg: {formatNumber(tokenData.avgBurnAmount)} CRA
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mt-6">
          <Button variant="outline" size="sm" asChild>
            <a href={`https://www.geckoterminal.com/apechain/pools/0xc9c2f86e542620daf12107d4b6eda37936efb903`} target="_blank" rel="noopener">
          ...
          <Button variant="outline" size="sm" asChild>
            <a href={`https://apechain.celatone.io/address/${CRA_TOKEN_ADDRESS}`} target="_blank" rel="noopener">
              <ExternalLink className="h-3 w-3 mr-1" />
              Explorer
            </a>
          </Button>
