...
const fmtCRA = (val: string | bigint) => {
  ...
  return new Intl.NumberFormat('en-US', { maximumFractionDigits: 2 }).format(cra)
}
...
export default function BurnCard({ nft, index, onActionComplete }: BurnCardProps) {
  ...
  const pingReady = data ? (data.lastPingTime === 0 || nowSec > data.lastPingTime + pingInterval) : false
  const pingTimeLeft = data ? Math.max(0, (data.lastPingTime + pingInterval) - nowSec) : 0

  // fetch burn split when waitHours changes
  useEffect(() => {
    let ignore=false
    getBurnSplit(waitHours).then(split=>{ if(!ignore) setBurnSplit(split) })
    return ()=>{ignore=true}
  }, [waitHours])
  ...
  if (data) {
    ...
    widgets.push(
      ...
      >
        <SatelliteDish className="w-2 h-2 mr-0.5 inline" /> {pingReady ? 'Ping ✓' : `${Math.ceil(pingTimeLeft / 60)}m`}
      </Badge>
    )
  }
  ...
  return (
    <>
    <div className={`flex flex-col h-full ${isLiteMode ? 'min-h-[380px]' : 'min-h-[420px]'}`}>
      <div className="flex-1 flex flex-col justify-between">
        {/* NFT visual layer */}
        <div className={`${data && Number(data.lockedCRA)===0 ? 'opacity-30 grayscale pointer-events-none' : ''}`}>
          <UnifiedNftCard
            imageSrc={nft.image}
            tokenId={tokenId}
            ...
            {(() => {const s = calcShares(); return (
              <>
                <div className="flex justify-between">
                  <span>👤 You</span>
                  <span className="font-semibold text-green-300 font-mono text-sm">{s.user}</span>
                </div>
                <div className="flex justify-between text-orange-300">
                  <span>🏦 Pool</span>
                  <span className="font-semibold font-mono text-sm">{s.pool}</span>
                </div>
                <div className="flex justify-between text-red-400">
                  <span>🔥 Burn</span>
                  <span className="font-semibold font-mono text-sm">{s.burn}</span>
                </div>
                <div className="text-center text-gray-400/70 pt-0.5 text-[10px]">Split {burnSplit.playerBps/100}% / {burnSplit.poolBps/100}% / {burnSplit.burnBps/100}%</div>
              </>
            )})()}
          </div>
        )}
      ...
      <div className="mt-auto w-full">
        <Button
          ...
          disabled={!isApeChain || isProcessing || !!data?.isInGraveyard || (!!data && Number(data.lockedCRA) === 0)}
          onClick={handleBurn}
        >
          Burn
        </Button>
    ...
    <style jsx global>{`
      @keyframes burnFade{
        0%{opacity:0;filter:brightness(2) saturate(1.5);}
        10%{opacity:0.9;}
        100%{opacity:0;transform:scale(0.8) rotate(2deg);}
      }
      .burn-overlay{
        background:radial-gradient(circle at center,rgba(255,200,0,0.6)0%,rgba(255,0,0,0.5)40%,transparent 80%);
        ...
        <AlertDialogContent className="bg-[#2f2b2b]/95 border border-red-500/30 text-gray-100 max-w-md text-[15px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2 text-orange-50">
              <p>Wait period: <span className="font-medium text-orange-300">{waitHours}h</span></p>
              <p>Locked CRA: <span className="font-mono text-yellow-300">{data.lockedCRA}</span></p>
              <p>Fee ({burnFeeBps/100}%): <span className="font-mono text-red-300">{calcFee()} CRA</span></p>
              {(() => {const s = calcShares(); return (
                <div className="pt-1 text-xs text-gray-300 space-y-0.5">
                  ...
                  <p>Pool receives: <span className="text-orange-300 font-mono">{s.pool}</span></p>
                  <p>Burned forever: <span className="text-red-400 font-mono">{s.burn}</span></p>
                </div>
              )})()}
              <div className="pt-2 text-xs text-gray-400">
                You will sign 3 transactions:<br/>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => { setDialogOpen(false); handleBurn(); }}>
              Confirm Burn
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
