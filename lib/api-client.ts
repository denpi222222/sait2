...
export async function rpcRequest<T>(
  ...
  try {
    const result = await ultraSmartFetch(requestData, {
      method: 'POST',
      headers: {
    ...
    if (result.error) {
      throw new ApiError(`RPC Error: ${result.error.message}`, result.error.code || 500)
    }
    ...
    throw new ApiError(`Network error: ${error.message}`, 503)
  }
}
...
export async function apiRequest<T>(url: string, options: RequestOptions = {}): Promise<T> {
  ...
  if (url.includes('alchemy.com')) {
    ...
    if (endpoint.type !== 'alchemy') {
      console.log(`🔄 Redirecting Alchemy request to ${endpoint.type}`)
...
export const blockchainApi = {
  ...
  getTransactionReceipt: (hash: string) => 
    rpcRequest<any>('eth_getTransactionReceipt', [hash]),
  
  // Get gas price
  getGasPrice: () => 
    rpcRequest<string>('eth_gasPrice'),
}
