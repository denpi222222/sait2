export const rarityLabels = ["","Common","Uncommon","Rare","Epic","Legendary","Mythic"] as const
...
export function labelToIndex(label:string){
  const idx = rarityLabels.findIndex(l=>l.toLowerCase()===label.toLowerCase())
  return idx>0?idx:1
}
...
export function getLabel(value:number|string){
  return typeof value==='number'?getRarityLabel(value):getRarityLabel(labelToIndex(value))
} 