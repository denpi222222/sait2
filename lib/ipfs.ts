 ...
 */
export function resolveIpfsUrl(url: string | undefined | null, opts: { webp?: boolean; width?: number } = {}): string {
  if (!url) return "";
 ...
 */
export function resolveIpfsUrlWithFallback(url: string | undefined | null): string[] {
  if (!url) return [];
